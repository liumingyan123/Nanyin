# -*- coding: utf-8 -*-
"""
train_musicmamba.py - 南音MusicMamba模型训练脚本
=================================================
...
功能概述：
  ...
  4. 保留print输出epoch编号、各类分项损失、全部指标得分，
     所有数据同步追加写入本地CSV文件 ./experiment_record_musicmamba.csv；
  5. 权重保存：验证集综合NOEF得分最高时，模型存入 ./checkpoints/musicmamba/，
     支持断点续训自动加载；
  6. 分层动态权重调度：
...
# ---- 导入自定义模块 ----
from models import NanyinMusicMamba
...
# ==============================================================================
# 辅助：因果自回归 target 对齐（训练目标 = 预测下一个 token）
# ==============================================================================

# 序列级真值字段：与模型输出逐位置对齐（pred[i] 对应 target[i+1]）
_SEQ_LEVEL_TARGET_FIELDS = ("target_tokens", "target_pitch", "target_duration", "target_ornament")


def shift_seq_targets(td: dict) -> dict:
    """
    将序列级真值右移一位（丢弃第一个 token），与因果自回归模型的 next-token
    预测输出（长度 L-1，输入 x[:-1]）对齐；全局级真值（与位置无关）保持不变。

    对齐关系：模型输入 x[:-1]，位置 i 的输出 = "给定 x[:i+1] 预测 x[i+1]"，
    对应的真值应为 x[i+1]，即 target_tokens[:, 1:]。
    若不做此对齐（旧行为 pred[i] vs target[i]），训练目标退化为双向完形/自复制，
    与自回归生成错位，导致续写 pitch_acc 极低。
    """
    return {
        k: (v[:, 1:] if k in _SEQ_LEVEL_TARGET_FIELDS else v)
        for k, v in td.items()
    }
...
# CSV 日志路径（MusicMamba 独立存储，便于与 BoYaTCN 横向对比）
CSV_LOG_PATH = os.path.join(_SCRIPT_DIR, "experiment_record_musicmamba.csv")

# 模型权重保存路径
CHECKPOINT_DIR = os.path.join(_SCRIPT_DIR, "checkpoints", "musicmamba")
...
EARLY_STOP_PATIENCE = 160        # 早停耐心：验证得分连续N轮不提升则终止。
                                 # 注意：验证集是模型从未见过的曲目，NOEF 很早就会饱和，
                                 # 若耐心过小会提前截断训练（λ权重尚未到位），导致模型欠拟合。
                                 # 160 > 剩余epoch，实际等价于跑满全部 epoch。
...
# ---- 模型结构参数（MusicMamba 特有） ----
VOCAB_SIZE = 128           # MIDI token 词表大小（0~127）
D_MODEL = 256              # 隐藏维度
NUM_SSM_LAYERS = 3         # SelectiveSSM 层数
D_STATE = 16               # SSM 状态维度
SSM_EXPAND = 2             # SSM 内部扩展因子
...
def validate_noef(
    ...
    for input_tokens, target_dict in val_loader:
        ...
        # 因果自回归对齐：序列级 target 右移一位（对应 next-token 预测）
        target_on_device = shift_seq_targets(target_on_device)

        # 前向传播（输入去掉最后一个 token，预测其后的每个 token）
        preds = model(input_tokens[:, :-1])
...
def init_csv_log(csv_path: str):
    ...
    # 定义CSV列名（与 train_boyatcn.py 完全一致，便于横向对比）
    headers = [
        "epoch",
...
def save_checkpoint(model: NanyinMusicMamba,
...
def load_checkpoint(model: NanyinMusicMamba,
                    optimizer: torch.optim.Optimizer,
                    save_dir: str,
                    filename: str = "best_model.pt",
...
def train(finals: Optional[Tuple[float, float, float, float, float]] = None,
    ...
    # ---- 2. 模型、优化器初始化 ----
    print("\n" + "=" * 60)
    print("[train] 初始化 NanyinMusicMamba 模型")
    print("=" * 60)
    ...
    print("[train] 训练配置")
    print("=" * 60)
    print(f"  模型架构:      MusicMamba (SelectiveSSM x {NUM_SSM_LAYERS})")
    print(f"  SSM状态维度:   {D_STATE}")
    print(f"  SSM扩展因子:   {SSM_EXPAND}")
    ...
    # ---- 7. 训练完成 ----
    print("\n" + "=" * 60)
    print(f"[train] MusicMamba 训练完成！共 {NUM_EPOCHS} 个epoch")
