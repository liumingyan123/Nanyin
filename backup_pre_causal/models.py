...
class OctaveConv1d(nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels, kernel_size=3, alpha_in=0.5, alpha_out=0.5, stride=1, padding=1):
        super().__init__()
        ...
        # 高频 -> 高频
        self.h2h = nn.Conv1d(in_h, out_h, kernel_size, stride, padding)
        # 高频 -> 低频 (下采样)
        self.h2l = nn.Conv1d(in_h, out_l, kernel_size, stride, padding)
        # 低频 -> 高频 (上采样)
        self.l2h = nn.Conv1d(in_l, out_h, kernel_size, stride, padding)
        # 低频 -> 低频
        self.l2l = nn.Conv1d(in_l, out_l, kernel_size, stride, padding)
    ...
    def forward(self, x_h, x_l):
        # 处理高频分量
        h2h = self.h2h(x_h)
        # 使用 interpolate 确保长度与 h2h 完全一致
        l2h = F.interpolate(self.l2h(x_l), size=h2h.size(-1), mode='nearest')
        out_h = h2h + l2h
        
        # 处理低频分量
        l2l = self.l2l(x_l)
        # 使用 interpolate 确保长度与 l2l 完全一致
        h2l = F.interpolate(self.h2l(x_h), size=l2l.size(-1), mode='nearest')
        out_l = l2l + h2l
...
class NanyinBoYaTCN(nn.Module):
    ...
    核心架构（完整 BoYaTCN 论文结构）：
      Token Embedding → 位置编码 → OctaveConv1d → MultiheadAttention
      → BiLSTM → TCN(膨胀因果卷积) → [序列级输出头 + 全局级输出头]
    ...
    def forward(self, input_tokens: torch.Tensor) -> dict:
        ...
        x_l = F.avg_pool1d(x_conv[:, self.in_h:, :], kernel_size=2, stride=2)
